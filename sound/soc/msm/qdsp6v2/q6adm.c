...
int srs_trumedia_open(int port_id, __s32 srs_tech_id, void *srs_params)
{
	...
	if (outband) {
		adm_params_->hdr.pkt_size = APR_PKT_SIZE(APR_HDR_SIZE, sizeof(
					      struct adm_cmd_set_pp_params_v5));
		adm_params_->payload_addr_lsw = lower_32_bits(
						this_adm.outband_memmap.paddr);
		adm_params_->payload_addr_msw = upper_32_bits(
						this_adm.outband_memmap.paddr);
		adm_params_->mem_map_handle = atomic_read(&this_adm.
