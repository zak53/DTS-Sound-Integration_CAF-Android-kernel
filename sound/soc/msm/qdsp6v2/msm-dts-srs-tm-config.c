...
static int set_port_id(int port_id)
{
	srs_port_id = port_id;
	return 0;
}
...
static void msm_dts_srs_tm_send_params(__s32 port_id, __u32 techs)
{
	...
	if (techs & (1 << SRS_ID_AEQ))
		srs_trumedia_open(port_id, SRS_ID_AEQ,
	...
	if (techs & (1 << SRS_ID_GEQ))
		srs_trumedia_open(port_id, SRS_ID_GEQ,
...
void msm_dts_srs_tm_init(int port_id)
{
	if (set_port_id(port_id) < 0) {
